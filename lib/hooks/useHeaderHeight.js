...
export function useHeaderHeight(selector = '#site-header', extraOffset = 16) {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const handler = () => {
      const measured = el?.offsetHeight ?? readCssHeaderH();
      const fromCss = readCssHeaderH();
      // якщо різняться — тут буде зсув
      if (measured !== fromCss) {
        console.warn('[header] MISMATCH measured:', measured, 'css:', fromCss);
      }
      setHeaderH(measured);
      document.documentElement.style.setProperty('--header-offset', `${measured + extraOffset}px`);
    };
    handler();
