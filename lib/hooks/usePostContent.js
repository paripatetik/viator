...
document.addEventListener(
  ...
  (e) => {
    ...
    if (e.touches.length === 1) {
      ...
      // якщо збільшено — пан, інакше — свайп
...
document.addEventListener(
  ...
  (e) => {
    ...
    if (tMode === "pinch" && e.touches.length === 2) {
      ...
      // зум відносно центру pinch (щоб не “тікав”)
      const lb = document.getElementById("viator-lightbox");
      ...
      if (rect) {
        ...
        // корекція pos так, щоб точка під пальцями лишалась стабільною
        posX = posX - cxRel * (scaleChange - 1);
        posY = posY - cyRel * (scaleChange - 1);
      }
      ...
      posX = pinchStartPosX + centerDx;
      posY = pinchStartPosY + centerDy;
...
document.addEventListener(
  ...
  { passive: true }
    ...
    // ── One-time CSS injection (avoid duplicates) ──
    ...
    if (typeof document !== "undefined" && !document.getElementById(STYLE_ID)) {
      ...
      style.textContent = `
        ...
        @media (max-width: 768px) {
          h2.viator-ribbon { font-size: 1.2rem; }
        }
...
figure.viator-figure img {
  cursor: pointer;
        ...
        blockquote.viator-quote::before { content: "“"; top: 0; left: 0; }
        blockquote.viator-quote::after { content: "”"; right: 20px; }

        blockquote.viator-quote p { margin: 0; }
        blockquote.viator-quote p + p { margin-top: 1px; }
  ...
  return { rawText, author, title, year };
      ...
      const citationHTML = (_match, num, loc = "", punct = "") => {
      ...
      // НЕ ЇМО пробіли перед “[”
      doc.body.innerHTML = doc.body.innerHTML.replace(
        /\[\s*(\d+)\s*(?:[,，;:]\s*([^\]]+?))?\s*]\s*([.,;:!?])?/g,
        (_all, n, loc, punct) => citationHTML("", n, loc || "", punct || "")
      );
        ...
        <div role="note"
          ...
          <div class="pointer-events-none absolute inset-y-0 left-0 w-3"
              style="background:linear-gradient(to right,
                        rgba(59,85,96,.45) 0 2px,
                        var(--color-viator-sky) 2px 100%
                      );"></div>
          ...
          ${
            title
              ? `<div class="text-lg sm:text-xl font-semibold text-slate-900">${title}</div>
                 ${
                   bodyHTML
                     ? `<div class="mt-2 text-[16px] sm:text-[17px] leading-relaxed text-slate-800">${bodyHTML}</div>`
                     : ""
                 }`
