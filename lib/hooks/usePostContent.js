    ...
    if (heading) {
      ...
      const citationHTML = (_match, num, punct = "") => {
        ...
        return `<sup id="cite-${idx}" class="relative align-super group"
                 style="font-size:0;margin-left:2px;margin-right:-2px;top:1px">
          <a href="#" onclick="return window.__viatorGoToRef(${idx})"
             class="inline-flex h-4 w-4 items-center justify-center rounded-sm
                    bg-[#94B4C1] text-[11px] leading-none font-bold text-black no-underline">
            ${idx}
          </a>
...
allLists.forEach((ul) => {
  ...
  ul.classList.add(
    "grid", "gap-5", "my-6", "pl-0", "list-none",
    "md:grid-cols-2", "[grid-auto-rows:1fr]"
  );
  ...
  lis.forEach((li, idx) => {
    ...
    const left = hasIcon
      ...
      : `<div class="shrink-0">
           <div class="inline-flex h-8 w-8 items-center justify-center rounded-lg
